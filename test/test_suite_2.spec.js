...
// There are special matchers for interacting with spies. This syntax has changed for Jasmine 2.0.
...
describe("A spy, when configured with an alternate implementation", function () {
	...
	beforeEach(function () {
		...
		spyOn(foo, "getBar").and.callFake(function (arguments_1, can, be, received) {
			return 1001;
		});
...
describe("A spy", function () {
	...
	it("can provide the context and arguments to all calls", function () {
		...
		expect(foo.setBar.calls.all()).toEqual([jasmine.objectContaining({
			object: foo,
			args: [123],
	...
	it("has a shortcut to the most recent call", function () {
		...
		expect(foo.setBar.calls.mostRecent()).toEqual(jasmine.objectContaining({
			object: foo,
			args: [456, "baz"],
			returnValue: undefined
		}));
	...
	it("has a shortcut to the first call", function () {
		...
		expect(foo.setBar.calls.first()).toEqual(jasmine.objectContaining({
			object: foo,
			args: [123],
			returnValue: undefined
		}));
